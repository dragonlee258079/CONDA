...
class CenterPivotConv4d(nn.Module):
    ...
    def prune(self, ct):
        ...
        ct_pruned = ct.view(bsz, ch, ha, wa, n, -1).index_select(5, self.idx).view(bsz, ch, ha, wa, n, self.len_h, self.len_w)
    ...
    def forward(self, x):
        if self.stride[2:][-1] > 1:
            ...
            out1 = x
        bsz, inch, ha, wa, n, hb, wb = out1.size()
        out1 = out1.permute(0, 4, 5, 6, 1, 2, 3).contiguous().view(-1, inch, ha, wa)
        out1 = self.conv1(out1)
        outch, o_ha, o_wa = out1.size(-3), out1.size(-2), out1.size(-1)
        out1 = out1.view(bsz, n, hb, wb, outch, o_ha, o_wa).permute(0, 4, 5, 6, 1, 2, 3).contiguous()

        bsz, inch, ha, wa, n, hb, wb = x.size()
        out2 = x.permute(0, 4, 2, 3, 1, 5, 6).contiguous().view(-1, inch, hb, wb)
        out2 = self.conv2(out2)
        outch, o_hb, o_wb = out2.size(-3), out2.size(-2), out2.size(-1)
        out2 = out2.view(bsz, n, ha, wa, outch, o_hb, o_wb).permute(0, 4, 2, 3, 1, 5, 6).contiguous()
